...
// ✅ FIXED: Defined OUTSIDE component — prevents re-mount on every keystroke
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ah-root { min-height: 100vh; display: flex; font-family: 'DM Sans', sans-serif; background: var(--bg); }

        .ah-sidebar {
          display: none; position: sticky; top: 0; height: 100vh;
          width: 300px; flex-shrink: 0; background: #080a14;
        ...
        .ah-sidebar-icon {
          ...
          margin-bottom: 24px; box-shadow: 0 4px 16px rgba(99,102,241,0.4);
        }
        ...
        .ah-sidebar-sub { font-size: 13px; color: rgba(255,255,255,0.45); line-height: 1.7; margin-bottom: 36px; }
        .ah-sidebar-tips { display: flex; flex-direction: column; gap: 12px; border-top: 1px solid rgba(255,255,255,0.07); padding-top: 28px; }
        .ah-tip { display: flex; align-items: flex-start; gap: 10px; }
        .ah-tip-dot { width: 6px; height: 6px; border-radius: 50%; background: #818cf8; flex-shrink: 0; margin-top: 6px; }
        ...
        .ah-main { flex: 1; display: flex; align-items: flex-start; justify-content: center; padding: 48px 24px; overflow-y: auto; }
        ...
        .ah-card-head { padding: 28px 36px 24px; border-bottom: 1px solid var(--border); background: linear-gradient(135deg, #fafbff 0%, #f5f3ff 100%); }
        .ah-card-head-top { display: flex; align-items: center; gap: 14px; margin-bottom: 4px; }
        .ah-head-icon { width: 38px; height: 38px; border-radius: 10px; background: linear-gradient(135deg, #6366f1, #4f46e5); display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 10px rgba(99,102,241,0.3); }
        .ah-head-icon svg { width: 18px; height: 18px; fill: white; }
        .ah-card-title { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 700; color: var(--ink); }
        ...
        .ah-field { display: flex; flex-direction: column; }
        ...
        .ah-input-wrap .fi { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 14px; height: 14px; color: var(--muted); pointer-events: none; }
        .ah-textarea-wrap { position: relative; }
        .ah-textarea-wrap .fi { position: absolute; left: 12px; top: 14px; width: 14px; height: 14px; color: var(--muted); pointer-events: none; }

        .ah-error { display: flex; align-items: center; gap: 5px; font-size: 12px; color: #ef4444; margin-top: 5px; }
        ...
        .ah-diff-pill {
          ...
          background: #fafbfd; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px;
          transition: all 0.18s; user-select: none;
        }
        .ah-diff-pill .dot { width: 7px; height: 7px; border-radius: 50%; }

        .ah-tag-box {
          min-height: 44px; padding: 6px 10px; border: 1.5px solid var(--border); border-radius: 10px;
          background: #fafbfd; display: flex; flex-wrap: wrap; gap: 6px; align-items: center;
          transition: border-color 0.2s, box-shadow 0.2s, background 0.2s; cursor: text;
        }
        .ah-tag-box:focus-within { border-color: var(--accent); background: #fff; box-shadow: 0 0 0 3px rgba(79,70,229,0.1); }
        .ah-tag { display: flex; align-items: center; gap: 5px; padding: 3px 10px; background: #ede9fe; border-radius: 100px; font-size: 12px; font-weight: 500; color: var(--accent); }
        .ah-tag button { background: none; border: none; cursor: pointer; color: #a78bfa; display: flex; padding: 0; transition: color 0.15s; }
        .ah-tag button:hover { color: var(--accent-dark); }
        .ah-tag-input { border: none; outline: none; background: transparent; font-family: 'DM Sans', sans-serif; font-size: 13.5px; color: var(--ink); flex: 1; min-width: 120px; }
        ...
        .ah-section-divider { height: 1px; background: var(--border); margin: 4px 0; }

        .ah-submit {
          width: 100%; padding: 13px; background: linear-gradient(135deg, #6366f1, #4f46e5);
          color: #fff; font-family: 'DM Sans', sans-serif; font-size: 14.5px; font-weight: 500;
          border: none; border-radius: 10px; cursor: pointer;
          display: flex; align-items: center; justify-content: center; gap: 8px;
          box-shadow: 0 2px 12px rgba(79,70,229,0.3);
          transition: opacity 0.2s, transform 0.15s, box-shadow 0.2s;
        }
        .ah-submit:hover:not(:disabled) { opacity: 0.92; transform: translateY(-1px); box-shadow: 0 4px 20px rgba(79,70,229,0.4); }
      ...
      <div className="ah-root">
        <aside className="ah-sidebar">
          ...
          <div className="ah-sidebar-content">
            ...
            <h2 className="ah-sidebar-title">Launch a new<br /><em>Challenge</em></h2>
            <p className="ah-sidebar-sub">Create a hackathon challenge for developers worldwide. Set the difficulty, timeline, and let the best builders compete.</p>
            <div className="ah-sidebar-tips">
              {[
                ...
                <div className="ah-tip" key={t.title}>
                  ...
                  <div className="ah-tip-text"><strong>{t.title}</strong>{t.sub}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              <form onSubmit={formik.handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  <div className="ah-diff-pills">
                    {Object.entries(DIFFICULTY_CONFIG).map(([level, cfg]) => (
                      <div
                        ...
                        style={formik.values.difficulty === level ? { borderColor: cfg.border, background: cfg.bg, color: cfg.color } : {}}
                      >
                ...
                <button type="submit" className="ah-submit" disabled={isLoading}>
                  {isLoading
                    ...
                    : <><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg> Publish Challenge</>
                  }
                </button>
              </form>
            </div>
